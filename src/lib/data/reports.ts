...
export interface StoredReport extends ReportSections {
  ...
  dateEnd: string;
  generatedAt: string;
  topicsData: Topic[];   // 快取的結構化話題分析（供圖表，免重跑 LLM）
}
...
export async function saveReport(brand: string, dateStart: string, dateEnd: string, s: ReportSections, topicsData: Topic[] = []): Promise<void> {
  const supa = createServiceClient();
...
export async function getLatestReport(brand: string): Promise<StoredReport | null> {
  ...
  return {
    ...
    topicsData: (r.topics_data as Topic[] | null) ?? [],
  };
}
