...
===SECTION:competitor===
競品態勢（250–400 字）：若無競品資料（上下文不含【競品對比】段落），直接寫「本期尚無競品資料，此段跳過」即可，不要編造。若有競品資料，必須包含：一、競爭格局：各品牌聲量份額與互動效率排名，指出我們處於領先/持平/落後。二、策略差距：競品做了什麼我們沒做的（具體話題/創作者/內容格式），含數字。三、機會窗口：1–2 個最具潛力的未開發機會（競品有高互動但我們零覆蓋的領域）。四、威脅信號：若競品近期有明顯增長或在我們的強項上發力，點出並建議應對。若某競品數據不足，標注「樣本不足，結論僅供參考」。`;
